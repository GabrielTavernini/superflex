...
class NoPositionalEncoding(nn.Module):
    """Identity positional encoding for DroPE (Dropped Positional Embeddings).
    
    Used during finetuning to drop positional embeddings and allow the model
    to learn position-agnostic representations. Based on the DroPE paper:
    ...
    def forward(self, x):
        """Return input unchanged - no positional encoding applied."""
        return x
...
class TransformerDecoder(nn.Module):
    def __init__(self, decoder_layer: TransformerDecoderLayer, n_layers, max_len, masked_attention, pos_encoding_type='sinusoidal'):
        ...
        # Choose between sinusoidal, learnable, and no positional encodings
        if pos_encoding_type == 'learnable':
            ...
            self.positional_encoding = NoPositionalEncoding(self.d_model, max_len)
        else:
            raise ValueError("pos_encoding_type must be 'sinusoidal', 'learnable', or 'none'")
